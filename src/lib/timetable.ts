...
// lib/timetableUtils.ts
import type { CellContent } from '../interfaces/types';
import type { Subject, TimetableEntry, TimetableDatabase } from '../interfaces/database';
import { generateTimeLabels } from './util';

export const dayLabels = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];

export const defaultBlockedTexts = [
  'break', 'short break', 'devotion', 'assembly', 'lunch', 'recess',
  'morning devotion', 'closing', 'games', 'sports', 'free period'
];

// Extract all cell data for logging/export
export const extractTimetableData = (
  ...
  for (let row = 0; row < 5; row++) { // 5 days
...
// Check if a cell is blocked (contains break text, etc.)
export const isCellBlocked = (cellContent: CellContent | undefined, blockedTexts: string[]): boolean => {
...
// Log timetable data to console
export const logTimetableData = (entries: TimetableEntry[]): void => {
  console.group('📅 Timetable Data Export');
  
  console.log('📊 Summary:');
  console.log(`Total entries: ${entries.length}`);
  console.log(`Entries with content: ${entries.filter(e => e.customText).length}`);
